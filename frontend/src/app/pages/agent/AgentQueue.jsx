...
export function AgentQueue() {
  ...
  const handleAcceptOrder = async (delivery) => {
    try {
      ...
      toast.success('Delivery accepted! Check Active Deliveries');
      await loadAvailableOrders();
    } catch (error) {
      console.error('Error accepting delivery:', error);
      const errorMessage = error.response?.data?.message || error.message || 'Failed to accept delivery';
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {availableOrders.map((delivery) => (
              <Card key={delivery.id} className="p-6 hover:shadow-lg transition-shadow">
                ...
                <Button
                  onClick={() => handleAcceptOrder(delivery)}
